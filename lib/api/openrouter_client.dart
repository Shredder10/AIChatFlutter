...
class OpenRouterClient {
  ...
  Future<void> init() async {
    ...
    // Определяем по токену, какой сервис использовать
    if (apiKey!.startsWith('sk-or-vv-')) {
      baseUrl = dotenv.env['BASE_URL_VSEGPT'];
    } else if (apiKey!.startsWith('sk-or-v1-')) {
      baseUrl = dotenv.env['BASE_URL_OPENROUTER'];
    } else {
      throw Exception('Unknown API key format: $apiKey');
    }

    if (baseUrl == null || baseUrl!.isEmpty) {
      throw Exception('Base URL not found in .env');
    }
